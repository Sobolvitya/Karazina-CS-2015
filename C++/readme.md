...
##### Замечания.
Для выполнения лабораторной вам надо уметь немного работать со строками. В частности, у класса `string` вам нужны следующие методы:

 - `string.length()` - возвращает длину строки

 - `string.substr(pos, len)` - возвращает подстроку длины len начиная с символа pos.
 
 - также можно получить символ строки по номеру, обращаясь к строке как к массиву, т.е. например `s[3]`. Заметьте, что символы в строке нумеруются начиная с нулевого.
